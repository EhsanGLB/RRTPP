...
int main(int argc, char *argv[])
{
    ...
    forAll(timeDirs, timeI)
    {
        ...
        Info<< "Writing renal replacement therapy to field " << RRTPP.name() << nl << endl;

        dimensionedScalar epsWSS = dimensionedScalar("epsWSS",TAWSS.dimensions(), 1e-64);
        OSI =  0.5 * ( 1 - mag(TAWSS) / (TAWSSMag+epsWSS) );
        RRTPP = 1 /(( (1 - 2.0*OSI) * TAWSSMag )+epsWSS);
        RRTPP.write();
        nTime++;
    }
